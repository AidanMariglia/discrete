...
\begin{document}
	...
	\ee
	\textcolor{blue}{\textbf{Aidan Mariglia, mariglia, 27/03/21}}\\
	
	\be
	\item{
		$L = \{a^nb^m \mid 1 \le n \le m \}$ \bigskip\\
		...
		$\Gamma = \{\bot, A\}$
		\be
		\item ((s, a, $\bot$), (i, A$\bot$))
		\item ((i, a, A), (i, AA))
		\item ((i, b, A), (i, $\epsilon$))
		\item ((i, b, $\bot$), (f, $\bot$))
		\item ((i, $\epsilon$, $\bot$), (f, $\bot$))
		\ee
	}
	\item{
		$L = \{a^nb^m \mid 1 \le n < m \}$ \bigskip\\
		$M = (Q, \Sigma, \Gamma, \delta, s, \bot, F)$\\
		$Q = \{s,i,f\}$\\
		$F = \{f\}$\\
		$\Sigma = \{a, b\}$\\
	...
	\item{
		$L = \{a^nb^m \mid 1 \le m \le n \}$ \bigskip\\
		$M = (Q, \Sigma, \Gamma, \delta, s, \bot, F)$\\
		$Q = \{s,i,f\}$\\
		$F = \{f\}$\\
		$\Sigma = \{a, b\}$\\
		...
		\item ((i, b, A), (i, $\epsilon$))
		\item ((i, $\epsilon$, $\bot$), (f, $\bot$))
		\item ((i, $\epsilon$, A), (f, $\bot$))
		\ee
	}
	\item{
		$L = \{a^nb^m \mid 1 \le m < n \}$ \bigskip\\
		$M = (Q, \Sigma, \Gamma, \delta, s, \bot, F)$\\
		$Q = \{s,i,f\}$\\
		$F = \{f\}$\\
		$\Sigma = \{a, b\}$\\
